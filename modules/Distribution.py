...
def uniform_gaussian(points, dist_size, sample_range, stddvt):
  # g = gaussian(points, stddvt)
  # u = uniform(points, dist_size // 8)#.to(g.device)
  u = (torch.rand(dist_size//8, points.shape[1], device=points.device) * 2 - 1) * sample_range
  g = points + (torch.randn_like(points, dtype=torch.float32) * stddvt)
